...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("FedoraShare-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #FedoraShareTEST2\r");
            Send(hSocket, "WHO #FedoraShareTEST2\r");
        } else {
            // randomly join #FedoraShare00-#FedoraShare05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #FedoraShare%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #FedoraShare%02d\r", channel_number).c_str());
        }
