...
bool ParseMoney(const char* pszIn, int64_t& nRet)
{
    ...
    for (; *p; p++)
        if (!isspace(*p))
            return false;
    if (strWhole.size() > 12) // guard against 63 bit overflow
        return false;
...
#else
    const char* pszModule = "FedoraShare";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\FedoraShare
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\FedoraShare
    // Mac: ~/Library/Application Support/FedoraShare
    // Unix: ~/.FedoraShare
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "FedoraShare";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "FedoraShare.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "FedoraShared.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong FedoraShare will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("FedoraShare"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
