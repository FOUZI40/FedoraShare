...
{
    resize(850, 550);
    setWindowTitle(tr("FedoraShare") + " - " + tr("Wallet"));
#ifndef Q_OS_MAC
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setToolTip(tr("Send coins to a FedoraShare address"));
...
//    tabGroup->addAction(mintingAction); disable minting tab
    ...
    aboutAction = new QAction(QIcon(":/icons/bitcoin"), tr("&About FedoraShare"), this);
    aboutAction->setStatusTip(tr("Show information about FedoraShare"));
    ...
    optionsAction->setStatusTip(tr("Modify configuration options for FedoraShare"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your FedoraShare addresses to prove you own them"));
    verifyMessageAction = new QAction(QIcon(":/icons/transaction_0"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified FedoraShare addresses"));
    secondAuthAction = new QAction(QIcon(":/icons/key"), tr("Second &auth..."), this);
    secondAuthAction->setStatusTip(tr("Second auth with your FedoraShare addresses"));
...
//    toolbar->addAction(mintingAction); disable minting tab
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("FedoraShare client") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("FedoraShare client"));
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to FedoraShare network", "", count));
}
...
{
    QString strTitle = tr("FedoraShare") + " - ";
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid FedoraShare address or malformed URI parameters."));
    }
